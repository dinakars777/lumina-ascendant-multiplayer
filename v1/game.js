...
function buildPlayer() {
    ...
    const bodyGeo = THREE.CapsuleGeometry
        ? new THREE.CapsuleGeometry(0.62, 1.2, 8, 12)
        : new THREE.CylinderGeometry(0.62, 0.66, 2.2, 14);
    ...
    state.entities.playerBody = body;
...
function startGame() {
    ...
    const diff = DIFFICULTY[state.selectedDifficulty] || DIFFICULTY.normal;
...
function startNight() {
    ...
    pushFeed('Nightfall. Ignite beacons to hold the line.', 'warn');
...
function applyPerk(perkId) {
    ...
    state.fireRadius = clamp(state.fireRadius, CONFIG.FIRE_MIN_RADIUS, getMaxFireRadius());
    AudioManager.play('perk');
}
...
function endGame(reason = 'The forest consumed your spark.') {
    ...
    const payout = grantEmbers(payoutBase, { useDifficulty: true, useCombo: false });
    ...
    if (title) title.textContent = 'EXTINGUISHED';
    if (message) message.textContent = `${reason} Survived ${survivedDays} day(s), score ${Math.floor(state.score)}, augments ${state.runPerks.length}.`;
    if (earned) earned.textContent = String(payout);

    pushFeed('Run ended. Upgrade and try a tougher route.', 'danger');
    updateHUD();
}
...
function updatePlayer(dt) {
    ...
    const input = new THREE.Vector3(
        (state.keys.d || state.keys.arrowright ? 1 : 0) - (state.keys.a || state.keys.arrowleft ? 1 : 0),
        0,
        (state.keys.s || state.keys.arrowdown ? 1 : 0) - (state.keys.w || state.keys.arrowup ? 1 : 0)
    );
    ...
    if (sprinting && input.lengthSq() > 0.06) {
    ...
    if (horizontalSpeed > 0.25) {
        const facing = Math.atan2(state.playerVelocity.x, state.playerVelocity.z);
        player.rotation.y = lerp(player.rotation.y, facing, 1 - Math.exp(-dt * 12));
    }
}
...
function performDash() {
    ...
    if (dir.lengthSq() < 0.001) {
        dir.set(Math.sin(state.entities.player.rotation.y), 0, Math.cos(state.entities.player.rotation.y));
    } else {
        dir.normalize();
    }
...
function spawnNightEnemies() {
    const count = Math.floor((CONFIG.NIGHT_ENEMY_BASE + state.day * CONFIG.NIGHT_ENEMY_GROWTH) * state.runtime.spawnMultiplier);
        ...
        const glowColor = archetype.id === 'leech'
            ...
            : archetype.id === 'charger'
                ...
                : archetype.id === 'titan'
                    ? 0xffb247
                    : 0xff4a65;
        const glowRange = archetype.id === 'titan' ? 24 : 12;
        const glow = new THREE.PointLight(glowColor, archetype.id === 'titan' ? 1.1 : 0.5, glowRange, 2);
        mesh.add(glow);

        world.scene.add(mesh);

        const baseHp = (54 + state.day * 10) * archetype.hpMul;
...
function updateEnemies(dt) {
    ...
    for (let i = state.entities.enemies.length - 1; i >= 0; i--) {
        ...
        if (archetype.id === 'titan' && distToPlayer < 10 && Math.random() < dt * 0.9) {
            triggerScreenShake(0.2);
        }

        if (steer.lengthSq() > 0.00001) steer.normalize();

        const baseSpeed = (state.runtime.enemySpeed + clamp(state.day * 0.22, 0, 3.4)) * archetype.speedMul;
        ...
        if (moveMag > 0.03) {
            ...
            mesh.rotation.y = lerp(mesh.rotation.y, rot, 1 - Math.exp(-dt * 10));
        }

        const flareDps = getFlareDamageAt(mesh.position.x, mesh.position.z);
        if (flareDps > 0) {
            enemy.hp -= flareDps * dt;
        }

        if (enemy.hp <= 0) {
            state.entities.enemies.splice(i, 1);
            if (mesh.parent) mesh.parent.remove(mesh);
            const emberGain = grantEmbers(archetype.reward, { useDifficulty: false, useCombo: true });
            state.score += emberGain;
            if (Math.random() < 0.2 + state.runMods.enemyDropBonus) state.shards += 1;
            if (state.runMods.killHeal > 0) state.health = clamp(state.health + state.runMods.killHeal, 0, 100);
            addMetric('enemiesKilled', 1);
            addMomentum(10 + archetype.reward);
            continue;
        }
...
function castPulseBlast() {
    ...
    const radius = CONFIG.PULSE_BASE_RADIUS * state.runMods.pulseRadiusMult;
    const damage = CONFIG.PULSE_BASE_DAMAGE * state.runMods.pulseDamageMult;
    const cooldown = CONFIG.PULSE_BASE_COOLDOWN * state.runMods.pulseCooldownMult;
    ...
    for (let i = 0; i < state.entities.enemies.length; i++) {
        ...
        enemy.hp -= damage * (0.45 + falloff * 0.85);
        enemy.velocity.add(new THREE.Vector3(ePos.x - playerPos.x, 0, ePos.z - playerPos.z).normalize().multiplyScalar(8 + falloff * 10));
        hits += 1;
    }
...
function updateVFX(dt) {
    ...
    if (flash) {
        if (state.vfx.flashTimer > 0) {
            ...
            flash.style.opacity = alpha.toFixed(2);
        } else {
    ...
    if (heartbeat) {
        if (state.phase === 'SURVIVE' && state.ghostProximity > 0.25) {
...
function triggerScreenShake(duration) {
    state.vfx.screenShakeTimer = Math.max(state.vfx.screenShakeTimer, duration);
}
...
function drawRadar() {
            ...
            : type === 'leech'
                ...
                : type === 'brute'
                    ...
                    : type === 'titan'
                        ? '#ffd171'
                        : '#ff6d7e';
        const size = type === 'titan' ? 3.4 : type === 'brute' ? 3 : 2.4;
    ...
    const heading = state.entities.player.rotation.y;
...
function updateHUD() {
    ...
    ui.phase.textContent = state.paused ? 'AUGMENT' : state.phase;
    ...
    if (ui.comboValue) ui.comboValue.textContent = `x${state.comboMultiplier.toFixed(2)}`;

    if (ui.objectiveText) {
        if (state.paused) {
            ui.objectiveText.textContent = 'Choose an augment to continue the run.';
        } else if (state.phase === 'GATHER') {
            ui.objectiveText.textContent = 'Gather wood and shards. Prepare your flare route.';
        } else if (state.phase === 'SURVIVE') {
            ui.objectiveText.textContent = `Ignite beacons (${state.beaconsLit}/${CONFIG.BEACON_COUNT}) and survive until dawn.`;
...
function loadProgress() {
    ...
    if (parsed) {
        ...
        state.inventory.upgrades = Array.isArray(inventory.upgrades)
            ? inventory.upgrades.filter((id) => id === 'blue_flame' || id === 'speed_boots')
            : [];
