...
function autoEquipRelic(id) {
    ...
    if (state.equippedRelics.length < 3) {
...
function rollRelicDrop(source = 'night') {
    ...
    let pick = Math.random() * Math.max(totalWeight, 0.0001);
...
function startGame() {
    ...
    state.wood = 2 + state.relicRuntime.startWoodBonus;
    state.shards = 1 + state.relicRuntime.startShardBonus;
...
function endGame(reason = 'The forest consumed your spark.', opts = {}) {
    ...
    if (message) message.textContent = opts.message || `${reason} Survived ${survivedDays} day(s), score ${Math.floor(state.score)}, augments ${state.runPerks.length}.`;
...
function spawnDirectorReinforcement(count = 1) {
    if (state.phase !== 'SURVIVE') return;

    const player = state.entities.player.position;
    let eliteCount = 0;
    for (let i = 0; i < count; i++) {
        const archetype = pickDirectorArchetype(state.day, state.director.intensity);
        let modifier = pickEliteModifier(state.day, archetype, false);
        if (!modifier && Math.random() < state.director.intensity * 0.18) {
...
function resolveEnemyDeath(enemy, cause = 'unknown') {
    ...
    if (Math.random() < shardDropChance) state.shards += 1;
...
function spawnHazard(type, x, z, radius, delay, damage, owner = null) {
    ...
    const telegraphColor = volatile ? 0xffa867 : 0xff5b79;
    ...
    for (let i = state.entities.hazards.length - 1; i >= 0; i--) {
        ...
        if (!hazard.exploded) {
            ...
            if (hazard.timer <= 0) {
                ...
                hazard.timer = 0.28;
                hazard.ring.material.color.setHex(hazard.type === 'volatile_blast' ? 0xfff0b2 : 0xffca8b);
                ...
                if (d <= hazard.radius) {
                    ...
                    const reason = hazard.type === 'volatile_blast' ? 'Volatile detonation' : 'Titan slam';
...
function drawRadar() {
    ...
    state.entities.enemies.forEach((enemy) => {
        ...
        const size = (type === 'titan' ? 3.4 : type === 'brute' ? 3 : 2.4) + (enemy.modifier ? 0.4 : 0);
    ...
    state.entities.hazards.forEach((hazard) => {
        if (!hazard.exploded) {
            const color = hazard.type === 'volatile_blast' ? '#ffc07d' : '#ff8a8a';
...
function updateHUD() {
    ...
    if (ui.objectiveText) {
        if (state.paused) {
            ...
            ui.objectiveText.textContent = `Gather wood and shards. Fire ${firePct}%.`;
        } else if (state.phase === 'SURVIVE') {
            const threatPct = Math.round((1 + state.pushStreak * CONFIG.PUSH_THREAT_STEP) * 100);
            ui.objectiveText.textContent = `Ignite beacons (${state.beaconsLit}/${CONFIG.BEACON_COUNT}). Fire ${firePct}% | Threat ${threatPct}%.`;
...
function loadProgress() {
    ...
    if (state.equippedRelics.length === 0 && state.inventory.relics.length > 0) {
        state.equippedRelics = state.inventory.relics.slice(0, 3);
    }
    state.equippedRelics = Array.from(new Set(state.equippedRelics)).slice(0, 3);
...
function saveProgress() {
    const payload = {
        ...
        inventory: {
            ...
            equippedRelics: Array.from(new Set(state.equippedRelics)).slice(0, 3)
        }
    };
