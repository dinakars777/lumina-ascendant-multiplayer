...
const UPGRADE_IDS = new Set(['blue_flame', 'speed_boots', 'pulse_stabilizer', 'trap_kit']);
...
function getMovementDirection() {
    ...
    TMP.camRight.crossVectors(WORLD_UP, TMP.camForward);
...
function spawnHazard(type, x, z, radius, delay, damage, owner = null) {
    ...
    const ring = new THREE.Mesh(
        new THREE.RingGeometry(radius * 0.72, radius * 0.92, 46),
        new THREE.MeshBasicMaterial({
            color: 0xff5b79,
            transparent: true,
            opacity: 0.28,
    ...
    for (let i = state.entities.hazards.length - 1; i >= 0; i--) {
        ...
        if (!hazard.exploded) {
            ...
            if (hazard.timer <= 0) {
                ...
                hazard.timer = 0.28;
                hazard.ring.material.color.setHex(0xffca8b);
                ...
                if (d <= hazard.radius) {
                    ...
                    applyPlayerDamage(hazard.damage * (0.65 + falloff * 0.65), 'Titan slam');
...
function drawRadar() {
    ...
    state.entities.hazards.forEach((hazard) => {
        if (!hazard.exploded) {
            plot(hazard.x, hazard.z, '#ff8a8a', 2.6);
        }
    });
